...
class MemberMixin:
    ...
    def _eval_list_member(self, obj: list, member: str) -> Any:
        ...
        if member in ('mapNths', 'mapDropEvery'): return ('builtin', lambda args, ctx, m=member: obj[::int(args[0])] if m == 'mapNths' else [x for i, x in enumerate(obj) if (i + 1) % int(args[0]) != 0])
