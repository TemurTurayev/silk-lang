...
class MemberMixin:
    ...
    def _eval_number_member(self, obj: int | float, member: str) -> Any:
        ...
        _onearg = {
            ...
            'polygonal': lambda a: int(obj) * ((int(a[0]) - 2) * int(obj) - (int(a[0]) - 4)) // 2, 'nthDigit': lambda a: int(str(abs(int(obj)))[int(a[0])]), 'digitAt': lambda a: int(str(abs(int(obj)))[::-1][int(a[0])]), 'rotateDigits': lambda a: (lambda s, n: int(s[n:] + s[:n]))(str(int(obj)), int(a[0]) % len(str(int(obj)))), 'truncateDigits': lambda a: int(str(int(obj))[:int(a[0])]), 'padDigits': lambda a: str(int(obj)).zfill(int(a[0])), 'digitHeadN': lambda a: [int(d) for d in str(abs(int(obj)))][:int(a[0])], 'digitTailN': lambda a: [int(d) for d in str(abs(int(obj)))][-int(a[0]):],
