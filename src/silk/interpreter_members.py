...
class MemberMixin:
    ...
    def _eval_list_member(self, obj: list, member: str) -> Any:
        ...
        _noarg = {
            ...
            'mapCompact': lambda: [x for x in obj if x], 'mapShuffle': lambda: random.sample(obj, len(obj)), 'mapDifference': lambda: [obj[i+1] - obj[i] for i in range(len(obj)-1)], 'mapCumSum': lambda: list(__import__('itertools').accumulate(obj)),
        }
        if member in _noarg:
            fn = _noarg[member]
            return ('builtin', lambda args, ctx: fn())
        ...
        if member == 'mapChunksOf': return ('builtin', lambda args, ctx: (lambda n: [obj[i:i+n] for i in range(0, len(obj), n)])(int(args[0])))
        ...
        if member == 'mapInterleave': return ('builtin', lambda args, ctx: (lambda a, b: [v for p in __import__('itertools').zip_longest(a, b) for v in p if v is not None])(obj, args[0]))
