...
class MemberMixin:
    ...
    def _eval_list_member(self, obj: list, member: str) -> Any:
        ...
        _noarg = {
            ...
            'mapFlipPairs': lambda: [obj[i+1] if i % 2 == 0 and i+1 < len(obj) else obj[i-1] if i % 2 == 1 else obj[i] for i in range(len(obj))],
        }
        if member in _noarg:
            fn = _noarg[member]
            return ('builtin', lambda args, ctx: fn())
    ...
    def _eval_number_member(self, obj: int | float, member: str) -> Any:
        ...
        _simple = {
            ...
            'digitMirror': lambda: int(str(abs(int(obj))) + str(abs(int(obj)))[::-1]), 'digitMin': lambda: min(int(d) for d in str(abs(int(obj)))),
        }
