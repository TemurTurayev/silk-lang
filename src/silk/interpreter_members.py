...
class MemberMixin:
    ...
    def _eval_list_member(self, obj: list, member: str) -> Any:
        ...
        if member in ('mapCartesian', 'mapSelectIndices'): return ('builtin', lambda args, ctx, m=member: [[a, b] for a in obj for b in args[0]] if m == 'mapCartesian' else [obj[i] for i in args[0] if 0 <= i < len(obj)])
