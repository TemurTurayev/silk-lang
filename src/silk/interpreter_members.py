...
class MemberMixin:
    ...
    def _eval_number_member(self, obj: int | float, member: str) -> Any:
        ...
        _simple = {
            ...
            'digitMirror': lambda: int(str(abs(int(obj))) + str(abs(int(obj)))[::-1]), 'digitMin': lambda: min(int(d) for d in str(abs(int(obj)))), 'digitMax': lambda: max(int(d) for d in str(abs(int(obj)))),
        }
