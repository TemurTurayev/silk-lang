...
class MemberMixin:
    ...
    def _eval_number_member(self, obj: int | float, member: str) -> Any:
        ...
        _onearg = {
            ...
            'primesBetween': lambda a: [p for p in range(int(obj), int(a[0])+1) if p >= 2 and all(p % i for i in range(2, int(p**0.5)+1))], 'digitContainsDigit': lambda a: str(int(a[0])) in str(abs(int(obj))), 'digitFirstN': lambda a: int(str(abs(int(obj)))[:int(a[0])]),
        }
        if member in _onearg:
            fn = _onearg[member]
            return ('builtin', lambda args, ctx: fn(args))
